...
class TelegramBot:
    ...
    def send_startup(self, custom_message: str = None):
        """Send bot startup notification."""
        self.start_time = time.time()
        msg = custom_message if custom_message else "🚀 *MODEL DM BOT STARTED*"
        self.send(
            f"{msg}\n"
    ...
    def send_liker_model_complete(self, model: str, likes_given: int, sender_account: str = ""):
        """Notify that a model comment liking target is complete."""
        sender = str(sender_account or "").strip().lstrip("@")
        sender_line = f"👤 IG Account: `@{sender}`\n" if sender else ""

        self.send(
            f"✅ *COMMENT LIKER COMPLETE*\n\n"
            f"🎯 Model: `@{model}`\n"
            f"{sender_line}"
            f"❤️ Likes given: {likes_given}"
        )

    def send_story_liker_model_complete(self, model: str, likes_given: int, sender_account: str = ""):
        """Notify that a model story liking target is complete."""
        sender = str(sender_account or "").strip().lstrip("@")
        sender_line = f"👤 IG Account: `@{sender}`\n" if sender else ""

        self.send(
            f"✅ *STORY LIKER COMPLETE*\n\n"
            f"🎯 Model: `@{model}`\n"
            f"{sender_line}"
            f"🎬 Story likes given: {likes_given}"
        )

    def send_story_like_milestone(self, total_likes: int, sender_account: str = "", model: str = ""):
        """Notify when story likes cross a milestone."""
        sender = str(sender_account or "").strip().lstrip("@")
        model_name = str(model or "").strip().lstrip("@")
        sender_line = f"👤 IG Account: `@{sender}`\n" if sender else ""
        model_line = f"🎯 Model: `@{model_name}`\n" if model_name else ""

        self.send(
            f"🎬 *STORY LIKE MILESTONE*\n\n"
            f"✅ Total story likes: {total_likes}\n"
            f"{sender_line}"
            f"{model_line}"
        )
    ...
    def _get_status_text(self) -> str:
        ...
        return (
            ...
            f"❤️ Likes Given: `{self.stats['likes_given']}`\n"
            f"🎬 Story Likes: `{self.stats.get('story_likes_given', 0)}`\n"
