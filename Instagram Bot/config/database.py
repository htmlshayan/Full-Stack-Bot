...
DEFAULT_DB_SETTINGS = {
    ...
    "LIKE_POSTS_COUNT": 5,
    "LIKE_PER_POST_COUNT": 10,
    "STORY_LIKE_MAX_FOLLOWERS": 50,
    "STORY_LIKE_LIKES_PER_ACCOUNT": 100,
    "STORY_LIKE_MAX_PER_PROFILE": 50,
    "STORY_LIKE_INCLUDE_HIGHLIGHTS": True,
    "STORY_LIKE_MAX_HIGHLIGHTS": 5,
}
...
def _migrate_like_event_log_schema(conn):
    columns = _table_columns(conn, "like_event_log")
    if not columns:
        return

    legacy_columns = {"target_username", "status"}
    if not (legacy_columns & columns):
        return

    id_primary = "SERIAL PRIMARY KEY" if USE_POSTGRES else "INTEGER PRIMARY KEY AUTOINCREMENT"

    conn.execute("DROP TABLE IF EXISTS like_event_log_new")
    conn.execute(
        f"""
            CREATE TABLE IF NOT EXISTS like_event_log_new (
                id {id_primary},
    ...
    if "post_url" in columns:
        post_url_expr = "COALESCE(post_url, '')"
    elif "target_username" in columns:
        post_url_expr = "COALESCE(target_username, '')"

    likes_count_expr = "1"
    if "likes_count" in columns:
        likes_count_expr = "COALESCE(likes_count, 1)"

    conn.execute(
        f"""
        INSERT INTO like_event_log_new (sender_account, model_username, post_url, likes_count, timestamp)
        SELECT sender_account, model_username, {post_url_expr}, {likes_count_expr}, timestamp
        FROM like_event_log
        """
    )

    conn.execute("DROP TABLE like_event_log")
    conn.execute("ALTER TABLE like_event_log_new RENAME TO like_event_log")
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_like_event_log_sender ON like_event_log(sender_account)"
    )
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_like_event_log_ts ON like_event_log(timestamp DESC)"
    )


def _ensure_like_event_log_columns(conn):
    _migrate_like_event_log_schema(conn)
    columns = _table_columns(conn, "like_event_log")
    if not columns:
        return

    if "post_url" not in columns:
        conn.execute("ALTER TABLE like_event_log ADD COLUMN post_url TEXT NOT NULL DEFAULT ''")

    if "likes_count" not in columns:
        conn.execute("ALTER TABLE like_event_log ADD COLUMN likes_count INTEGER NOT NULL DEFAULT 1")

    if "likes_count" in _table_columns(conn, "like_event_log"):
        conn.execute("UPDATE like_event_log SET likes_count = 1 WHERE likes_count IS NULL")
...
def init_db():
    ...
    try:
        ...
        conn.execute("DROP TABLE IF EXISTS engagement_log")

        # Like Event Log Table
        conn.execute(f"""
            CREATE TABLE IF NOT EXISTS like_event_log (
                id {id_primary},
                sender_account TEXT NOT NULL,
                model_username TEXT NOT NULL,
                post_url TEXT NOT NULL,
        ...
        # Story Like Event Log Table
        conn.execute(f"""
            CREATE TABLE IF NOT EXISTS story_like_event_log (
                id {id_primary},
                sender_account TEXT NOT NULL,
                model_username TEXT NOT NULL,
        ...
        """)
        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_story_like_event_log_sender ON story_like_event_log(sender_account)"
        )
        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_story_like_event_log_ts ON story_like_event_log(timestamp DESC)"
        )
        ...
        _ensure_like_event_log_columns(conn)
...
def log_like_event(sender_account: str, model_username: str, post_url: str, likes_count: int):
    """Log a like event to the database."""
    conn = _get_connection()
    try:
        conn.execute(
            """
            INSERT INTO like_event_log (sender_account, model_username, post_url, likes_count, timestamp)
            VALUES (?, ?, ?, ?, ?)
            """,
            (
                str(sender_account or "").strip(),
                str(model_username or "").strip(),
...
def get_liker_dashboard_metrics() -> dict:
    """Return lightweight dashboard like metrics for today and total."""
    day_start_dt = datetime.now().replace(hour=0, minute=0, second=0, microsecond=0)
    day_start_iso = day_start_dt.isoformat(timespec="seconds")
    ...
    try:
        today_row = conn.execute(
            "SELECT SUM(likes_count) AS c FROM like_event_log WHERE timestamp >= ?",
            (day_start_iso,),
        ).fetchone()
        likes_today = int(today_row["c"] if today_row and today_row["c"] is not None else 0)

        lifetime_row = conn.execute(
            "SELECT SUM(likes_count) AS c FROM like_event_log"
        ).fetchone()
        lifetime_likes = int(lifetime_row["c"] if lifetime_row and lifetime_row["c"] is not None else 0)

        latest_row = conn.execute(
            "SELECT timestamp FROM like_event_log ORDER BY id DESC LIMIT 1"
        ).fetchone()
        last_like_at = ""
        if latest_row:
            last_like_at = str(latest_row["timestamp"] or "").strip().replace("T", " ")

        return {
            "likes_today": likes_today,
            "lifetime_likes": lifetime_likes,
...
def log_story_like_event(sender_account: str, model_username: str, story_url: str, likes_count: int):
    """Log a story-like event to the database."""
    conn = _get_connection()
    try:
        conn.execute(
            """
            INSERT INTO story_like_event_log (sender_account, model_username, story_url, likes_count, timestamp)
            VALUES (?, ?, ?, ?, ?)
            """,
            (
                str(sender_account or "").strip(),
                str(model_username or "").strip(),
                str(story_url or "").strip(),
                int(likes_count or 0),
                datetime.now().isoformat(timespec="seconds"),
            ),
        )
...
def get_story_liker_dashboard_metrics() -> dict:
    """Return lightweight dashboard story-like metrics for today and total."""
    day_start_dt = datetime.now().replace(hour=0, minute=0, second=0, microsecond=0)
    day_start_iso = day_start_dt.isoformat(timespec="seconds")
    ...
    try:
        today_row = conn.execute(
            "SELECT SUM(likes_count) AS c FROM story_like_event_log WHERE timestamp >= ?",
            (day_start_iso,),
        ).fetchone()
        story_likes_today = int(today_row["c"] if today_row and today_row["c"] is not None else 0)

        lifetime_row = conn.execute(
            "SELECT SUM(likes_count) AS c FROM story_like_event_log"
        ).fetchone()
        lifetime_story_likes = int(
            lifetime_row["c"] if lifetime_row and lifetime_row["c"] is not None else 0
        )

        latest_row = conn.execute(
            "SELECT timestamp FROM story_like_event_log ORDER BY id DESC LIMIT 1"
        ).fetchone()
        last_story_like_at = ""
        if latest_row:
            last_story_like_at = str(latest_row["timestamp"] or "").strip().replace("T", " ")

        return {
            "story_likes_today": story_likes_today,
            "lifetime_story_likes": lifetime_story_likes,
            "last_story_like_at": last_story_like_at,
        }
    finally:
        conn.close()
